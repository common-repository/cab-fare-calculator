...
class BookingHelper {
	...
	public static function get_car_details( $id = 0 ) {
		...
		$row = $wpdb->get_row(
			$wpdb->prepare(
				"SELECT * FROM {$wpdb->prefix}tblight_cars WHERE id = %d",
				(int) $id
			)
		);
	...
	public static function get_payment_details( $id = 0 ) {
		...
		$row = $wpdb->get_row(
			$wpdb->prepare(
				"SELECT id, title, text, payment_element, payment_params FROM {$wpdb->prefix}tblight_paymentmethods WHERE id = %d",
				(int) $id
			)
		);
	...
	public static function get_order_by_id( $id = 0 ) {
		...
		$row = $wpdb->get_row(
			$wpdb->prepare(
				"SELECT a.*, pm.title AS payment_method FROM {$wpdb->prefix}tblight_orders AS a LEFT JOIN {$wpdb->prefix}tblight_paymentmethods AS pm ON pm.id = a.payment WHERE a.id = %d",
				(int) $id
			)
		);
	...
	public static function get_order_by_order_number( $order_number = '' ) {
		...
		if ( $order_number != '' ) {
			$row = $wpdb->get_row(
				$wpdb->prepare(
					"SELECT a.*, pm.title AS payment_method FROM {$wpdb->prefix}tblight_orders AS a LEFT JOIN {$wpdb->prefix}tblight_paymentmethods AS pm ON pm.id = a.payment WHERE a.order_number = %s",
					$order_number
				)
			);
	...
	public static function get_order_car( $order, $exclude_url = false ) {
		if ( ! empty( $order ) ) {
			if ( $order->custom_car != '' ) {
				...
				$row = $wpdb->get_row(
					$wpdb->prepare(
						"SELECT id, title FROM {$wpdb->prefix}tblight_cars WHERE id = %d",
						(int) $order->vehicletype
					)
				);
